...
/** Набор тегов книги по трём измерениям (как их отдаёт каталог сервера). */
export interface ServerTagSet {
  classes: string[];
  subjects: string[];
  categories: string[];
}
...
export interface BookMeta {
  ...
  serverId?: string;
  /**
   * Подпись тегов (классы/предметы/категории), с которой книгу последний раз
   * публиковали на сервер. Нужна, чтобы отличать «синхронизировано» от «теги
   * изменены локально → нужно обновить на сервере» (кнопка на карточке).
   */
  serverSynced?: string;
  /**
   * Теги, которые каталог сервера отдавал по этой книге при последней сверке.
   * Хранятся, чтобы применять к локальной копии только ИЗМЕНЕНИЯ на сервере:
   * иначе снятый вручную тег возвращался бы при каждом открытии каталога.
   */
  serverTags?: ServerTagSet;
  /**
   * Масштаб fixed-layout (PDF/CBZ) в процентах поверх вписывания (100 — как
   * вписано). Запоминается на книгу: удобный масштаб учебника не сбрасывается.
   */
  zoomPct?: number;
