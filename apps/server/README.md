...
| `CHITALKA_CLIENT_LOGS` | `client-logs` рядом с БД | Папка журналов клиентов: сюда пишет `POST /api/client-logs` (веб, десктоп, Android) |
...
- `POST /api/client-logs` — приём журнала с устройства: `{ context, entries[] }` (записи `@reader/core`). **Открыт без авторизации** — клиент падает и до входа, а такие логи нужнее всего. Дописывается в `<CHITALKA_CLIENT_LOGS>/<дата>-<платформа>-<сессия>.ndjson` (файл режется по 32 МБ, максимум 5000 записей в пачке, тело до 16 МБ). Ошибки уровня `error` дополнительно поднимаются в лог сервера как `warn`. Ответ `204`.
- `GET /api/client-logs` — список файлов журналов (`{dir, files:[{file,size,modified}]}`), свежие сверху. Только admin.
- `GET /api/client-logs/{file}` — скачать файл журнала (NDJSON). Только admin.
